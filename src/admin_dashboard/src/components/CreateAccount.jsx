...
export default function CreateAccount({ onSave, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (role === "doctor") {
      for (const item of workSchedule) {
        if (!item.day || !item.start || !item.end) {
          alert("Vui lòng điền đầy đủ lịch làm việc hoặc xóa các mục trống");
          return;
        }
      }
    }
      ...
      <form onSubmit={handleSubmit} className="ca-form">
        ...
        {role === "doctor" && (
          <>
            ...
            {workSchedule.map((item, index) => (
              ...
              >
                <select
                  value={item.day}
                  onChange={(e) =>
                    updateWorkScheduleItem(index, "day", e.target.value)
                  }
                  required
                >
                ...
                <select
                  value={item.start}
                  onChange={(e) =>
                    updateWorkScheduleItem(index, "start", e.target.value)
                  }
                  required
                >
                ...
                <select
                  value={item.end}
                  onChange={(e) =>
                    updateWorkScheduleItem(index, "end", e.target.value)
                  }
                  required
                >
                ...
                <button
                  type="button"
                  onClick={() => removeWorkScheduleItem(index)}
                  title="Xóa lịch"
                >
                  ❌
                </button>
            ...
            <button type="button" onClick={addWorkSchedule}>
              ➕ Thêm lịch
            </button>
